 ...
 * @package App\Services\Sms\Providers
 *
 * Пример класса взят с офф сайта SmsAero, и немного видоизменен, чтобы он реализовал ProviderInterface
 */
final class SmsAeroProvider implements ProviderInterface
{
    ...
    private string $sign; //Подпись по умолчанию

    protected array $numbers;
    protected string $message;
    protected string $channel;

    public function __construct($email, $api_key, $sign = 'SMS Aero', string $channel = self::DEFAULT_CHANNEL)
    {
        $this->email = $email;
    ...
    {
        $this->defaultSend($this->numbers, $this->message, $this->channel);
    }
    ...
    /**
     * Отправка сообщения
     * @param $number array  - Номер телефона(ов)
     ...
     * @return array
     */
    public function defaultSend($number, $text, $channel, $dateSend = null, $callbackUrl = null){
